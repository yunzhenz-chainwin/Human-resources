...
| 文件 | 內容 |
|---|---|
| [docs/00-文件導覽與保留原則.md](docs/00-文件導覽與保留原則.md) | 文件閱讀順序、主來源、不可刪除項目、專題 Runbook 與二進位快照保留規則 |
...
| [docs/11-人才職缺媒合試行方案.md](docs/11-人才職缺媒合試行方案.md) | 媒合引擎現況、準確率強化、媒合評估報表、下一步 |
| [docs/12-後續工作與延伸建議.md](docs/12-後續工作與延伸建議.md) | 後續規劃總表（待辦＋做法合併）：每項含說明／價值、做法與製作順序、估時、完成定義、相依、風險，及波次與主管摘要 |
| [docs/13-結構化面試評分與盲評操作規格.md](docs/13-結構化面試評分與盲評操作規格.md) | 逐題 1–5 分、草稿／提交／重開、修訂軌跡、雙方盲評及 UAT 阻擋項 |
