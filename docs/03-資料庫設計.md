...
**文件版本**：v1.1（2026-08-05）｜**資料庫**：PostgreSQL 16
...
| `questions`、`personalization_basis` | 題目、目的、追問、來源及產題依據的 JSON 快照 |
| `generation_mode`、`provider`、`model_name` | Gemini／規則式產生模式及供應者中繼資料 |
...
| `summary`、`recommendation`、`overall_rating` | 整體總評；只有完整正式提交可進入 `completed` |
...
### `users`
`id、username UNIQUE、password_hash、display_name、email、department_id FK、role（admin/it/hr/manager）、is_active、登入鎖定與 token 失效欄位、created_at`
