...
const analysisCandidateId = ref<number | null>(null)
const error = ref('')
const notice = ref('')
const form = reactive({
  title: '',
...
const analysisJobs = computed(() => workspace.value?.jobs.map(item => item.requisition) || [])
const activeAnalysisApplicant = computed<DepartmentApplicantDto | null>(() => {
  if (analysisCandidateId.value === null) return null
  for (const job of workspace.value?.jobs || []) {
    const applicant = job.applicants.find(item => item.candidate.id === analysisCandidateId.value)
...
function openCandidateAnalysis(applicant: DepartmentApplicantDto) {
  analysisCandidateId.value = applicant.candidate.id
}

function closeCandidateAnalysis() {
  analysisCandidateId.value = null
}
...
<template>
  <section class="department-page">
          ...
          <section class="applicants-section">
            ...
            <div v-if="selectedJob.applicants.length" class="applicant-list">
              <article v-for="item in selectedJob.applicants" :key="item.application_id">
                ...
                <footer><span>應徵來源：{{ item.application_source }}</span><span>投遞時間：{{ date(item.applied_at) }}</span><span>Application #{{ item.application_id }}</span><button type="button" :data-testid="`department-candidate-analysis-${item.candidate.id}`" @click="openCandidateAnalysis(item)">職位分析</button></footer>
              </article>
            </div>
            <div v-else class="department-empty"><strong>這項職缺目前沒有應徵者</strong><p>主管可至「新增人才」選擇此職缺並上傳履歷；確認入庫後，人才會自動顯示在這裡。</p></div>
          </section>
        </template>
    ...
    <div v-if="activeAnalysisApplicant" class="analysis-drawer-backdrop" @click.self="closeCandidateAnalysis" @keydown.esc="closeCandidateAnalysis">
      <aside class="analysis-drawer" role="dialog" aria-modal="true" aria-labelledby="department-analysis-title" data-testid="department-candidate-analysis-drawer">
        <header>
          <div><small>DE-IDENTIFIED ROLE ANALYSIS</small><h2 id="department-analysis-title">{{ activeAnalysisApplicant.candidate.name }}的職位分析</h2><p>{{ activeAnalysisApplicant.candidate.code }} · 僅使用 HR 已核准的去識別化履歷</p></div>
          <button type="button" aria-label="關閉職位分析" @click="closeCandidateAnalysis">×</button>
        </header>
        <div class="analysis-drawer-body">
          <CandidateAnalysisPanel
            :key="activeAnalysisApplicant.candidate.id"
            :candidate="activeAnalysisApplicant.candidate"
            :jobs="analysisJobs"
            role="manager"
