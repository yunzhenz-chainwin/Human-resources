...
const form = reactive({ title: '', description: '', page: '', category: 'other' as IssueCategory, severity: 'medium' as IssueSeverity, status: 'open' as IssueStatus, progress_percent: 0, expected_completion_date: '', reproduction_steps: '', resolution_notes: '' })
const quickForm = reactive({ title: '', category: 'other' as IssueCategory, status: 'open' as IssueStatus, expected_completion_date: '' })
...
const categoryLabel: Record<IssueCategory, string> = { feature: '新功能', optimization: '優化項目', bug: '問題修復', maintenance: '維護作業', other: '其他' }
const visibleIssues = computed(() => categoryFilter.value === 'all' ? issues.value : issues.value.filter(item => item.category === categoryFilter.value))
const filteredIssues = computed(() => statusFilter.value === 'all' ? visibleIssues.value : visibleIssues.value.filter(item => item.status === statusFilter.value))
const categoryCounts = computed(() => Object.fromEntries((Object.keys(categoryLabel) as IssueCategory[]).map(key => [key, issues.value.filter(item => item.category === key).length])) as Record<IssueCategory, number>)
...
function openIssue(issue?: SystemIssue) {
  ...
  Object.assign(form, issue ? { title: issue.title, description: issue.description, page: issue.page, category: issue.category || 'other', severity: issue.severity, status: issue.status, progress_percent: issue.progress_percent, expected_completion_date: issue.expected_completion_date || '', reproduction_steps: issue.reproduction_steps || '', resolution_notes: issue.resolution_notes || '' } : { title: '', description: '', page: '', category: 'other', severity: 'medium', status: 'open', progress_percent: 0, expected_completion_date: '', reproduction_steps: '', resolution_notes: '' })
  dialog.value = true
}
async function save() {
...
async function quickAdd() {
  ...
  saving.value = true; error.value = ''
  try {
    await adminApi.createSystemIssue({ title: quickForm.title.trim(), description: '待補充項目說明', page: 'IT 工作項目', category: quickForm.category, severity: 'medium', status: quickForm.status, progress_percent: 0, expected_completion_date: quickForm.expected_completion_date || null, reproduction_steps: null, resolution_notes: null })
    Object.assign(quickForm, { title: '', category: 'other', status: 'open', expected_completion_date: '' }); await load()
  } catch (cause) { error.value = cause instanceof Error ? cause.message : '無法新增工作項目' }
  finally { saving.value = false }
}
async function removeIssue(issue: SystemIssue) {
  if (!window.confirm(`確定刪除「${issue.title}」嗎？`)) return
  saving.value = true; error.value = ''
  try { await adminApi.deleteSystemIssue(issue.id); await load() }
  catch (cause) { error.value = cause instanceof Error ? cause.message : '刪除工作項目失敗' }
  finally { saving.value = false }
}
...
<template>
  <section class="ops panel">
    ...
    <div v-if="view === 'issues'" class="ops-body"><section class="progress-overview"><div class="overall-ring"><strong>{{ averageProgress }}%</strong><small>整體完成</small></div><div class="overview-track"><div class="track-line"><i :style="{width: `${averageProgress}%`}"></i></div><div class="overview-counts"><span>完成 {{ resolvedCount }}</span><span>待處理 {{ unresolvedCount }}</span><b>共 {{ issues.length }} 項</b></div></div><div class="category-progress"><div v-for="(label, key) in categoryLabel" :key="key"><span>{{ label }}</span><small>{{ categoryCounts[key] }} 項</small><i><em :style="{width: `${issues.length ? Math.round(categoryCounts[key] / issues.length * 100) : 0}%`}"></em></i></div></div></section><form class="quick-add" @submit.prevent="quickAdd"><b>＋ 新增項目</b><input v-model="quickForm.title" placeholder="工作項目標題…" required><select v-model="quickForm.category"><option v-for="(label, key) in categoryLabel" :key="key" :value="key">{{ label }}</option></select><select v-model="quickForm.status"><option value="open">待辦</option><option value="investigating">進行中</option><option value="resolved">完成</option></select><input v-model="quickForm.expected_completion_date" type="date"><button class="button primary" :disabled="saving">新增</button></form>
      ...
      <div class="issue-toolbar"><div class="category-filters"><button :class="{active: categoryFilter === 'all'}" @click="categoryFilter = 'all'">全部（{{ issues.length }}）</button><button v-for="(label, key) in categoryLabel" :key="key" :class="{active: categoryFilter === key}" @click="categoryFilter = key">{{ label }}（{{ categoryCounts[key] }}）</button></div><span class="issue-count">目前顯示 {{ visibleIssues.length }} 筆</span></div>
      <div class="issue-list">
        <article v-for="issue in visibleIssues" :key="issue.id" class="issue-card" role="button" tabindex="0" :aria-label="`編輯${categoryLabel[issue.category]}：${issue.title}，目前進度 ${issue.progress_percent}%`" @click="openIssue(issue)" @keydown.enter.prevent="openIssue(issue)" @keydown.space.prevent="openIssue(issue)">
          <div class="issue-heading"><div><span class="category-badge">{{ categoryLabel[issue.category] }}</span><span class="severity" :data-severity="issue.severity">{{ severityLabel[issue.severity] }}</span><strong>{{ issue.title }}</strong></div><b>{{ issue.progress_percent }}%</b><span class="issue-actions"><button type="button" @click.stop="openIssue(issue)">編輯</button><button type="button" class="danger" @click.stop="removeIssue(issue)">刪除</button></span></div>
...
<style scoped>
.progress-overview{display:grid;grid-template-columns:100px 1fr 2fr;gap:16px;align-items:center;padding:16px;border:1px solid #dce9e5;border-radius:12px;background:#f5faf8;margin-bottom:12px}.overall-ring{width:76px;height:76px;border-radius:50%;display:flex;flex-direction:column;justify-content:center;align-items:center;background:#e4f2ed;border:7px solid #43bc73;color:#1d6658}.overall-ring strong{font-size:18px}.overall-ring small{font-size:8px}.track-line{height:10px;border-radius:10px;background:#dce5e4;overflow:hidden}.track-line i{display:block;height:100%;background:#27b95e}.overview-counts{display:flex;gap:14px;margin-top:8px;font-size:9px;color:#56706b}.category-progress{display:grid;grid-template-columns:repeat(3,1fr);gap:8px}.category-progress div{font-size:8px;color:#49645f}.category-progress small{float:right}.category-progress i{display:block;height:5px;margin-top:4px;border-radius:5px;background:#dfe8e7;overflow:hidden}.category-progress em{display:block;height:100%;background:#39af8a}.quick-add{display:flex;align-items:center;gap:7px;padding:10px;border:1px solid var(--line);border-radius:10px;background:#fff;margin-bottom:12px}.quick-add b{font-size:10px}.quick-add input,.quick-add select{min-width:0;border:1px solid var(--line);border-radius:7px;padding:8px;font-size:9px}.quick-add input:first-of-type{flex:1}
...
.issue-toolbar{display:flex;align-items:center;justify-content:space-between;gap:8px;margin:0 0 10px;padding:8px 0}.category-filters{display:flex;flex-wrap:wrap;gap:5px}.category-filters button{border:1px solid var(--line);background:#fff;border-radius:999px;padding:5px 9px;color:var(--muted);font-size:8px}.category-filters button.active{background:#e6f3ef;color:#1f655e;border-color:#8fbdb6;font-weight:700}.issue-count{font-size:8px;color:var(--muted)}.category-badge{display:inline-block;padding:3px 6px;border-radius:5px;background:#edf2ff;color:#3d5e9b;font-size:8px}
.progress-overview{display:grid;grid-template-columns:104px minmax(220px,1fr) minmax(280px,1.4fr);gap:16px;align-items:center;margin-bottom:14px;padding:16px 18px;border:1px solid #dce8e5;border-radius:12px;background:linear-gradient(135deg,#f5faf8,#fff)}.overall-ring{width:82px;height:82px;border-radius:50%;display:grid;place-items:center;align-content:center;background:conic-gradient(#26bf67 var(--progress,0%),#26364a 0);position:relative;color:#18b95a}.overall-ring:after{content:'';position:absolute;inset:9px;border-radius:50%;background:#fff}.overall-ring strong,.overall-ring small{position:relative;z-index:1}.overall-ring strong{font-size:18px}.overall-ring small{font-size:8px;color:#687a75}.overview-track .track-line,.category-progress i{display:block;height:8px;border-radius:99px;background:#e5eeeb;overflow:hidden}.overview-track .track-line i,.category-progress em{display:block;height:100%;border-radius:inherit;background:linear-gradient(90deg,#3c8dd8,#25bd66)}.overview-counts{display:flex;gap:14px;margin-top:9px;font-size:9px;color:#71827e}.overview-counts b{color:#286b63}.category-progress{display:grid;grid-template-columns:repeat(3,1fr);gap:9px 13px}.category-progress span,.category-progress small{display:block;font-size:8px;color:#60736f}.category-progress small{float:right;margin-top:-12px}.category-progress i{height:5px;margin-top:5px}.quick-add{display:flex;align-items:center;gap:8px;margin-bottom:14px;padding:10px 12px;border:1px solid #d9e6e2;border-radius:10px;background:#f7faf9}.quick-add b{white-space:nowrap;font-size:10px;color:#355c56}.quick-add input,.quick-add select{height:34px;min-width:0;border:1px solid #d3e1dd;border-radius:7px;padding:0 9px;background:#fff;color:#355c56;font-size:10px}.quick-add input:first-of-type{flex:1}.quick-add input[type=date]{width:125px}.quick-add select{width:105px}.quick-add .button{height:34px}@media(max-width:900px){.progress-overview{grid-template-columns:90px 1fr}.category-progress{grid-column:1/-1}.quick-add{flex-wrap:wrap}.quick-add input:first-of-type{flex-basis:100%}}@media(max-width:560px){.progress-overview{grid-template-columns:1fr}.overall-ring{margin:auto}.overview-counts{flex-wrap:wrap}.category-progress{grid-template-columns:1fr 1fr}.quick-add>*{width:100%!important;flex-basis:100%!important}}
.progress-overview{grid-template-columns:88px minmax(220px,1fr) minmax(260px,1.2fr);padding:14px 16px;border-radius:8px;background:#fff;box-shadow:none}.overall-ring{width:72px;height:72px;background:#edf3f1;color:#216b63}.overall-ring:after{inset:7px;background:#fff}.overall-ring strong{font-size:16px}.overview-track .track-line{height:6px}.overview-track .track-line i,.category-progress em{background:#4c8f84}.category-progress{gap:8px 12px}.quick-add{padding:9px 11px;border-radius:8px;background:#fff}.quick-add input,.quick-add select{border-radius:5px}.issue-card{border-radius:6px;box-shadow:none}.issue-card:hover{transform:none;background:#fbfcfc}.issue-dashboard article{border-radius:7px;background:#fff;box-shadow:none}.category-filters button{border-radius:5px;background:#fff}
.ops:has(.issue-list){background:#111b2d;border-color:#263650;color:#dbe5f2}.ops:has(.issue-list) .ops-header,.ops:has(.issue-list) .ops-nav{border-color:#2a3a55}.ops:has(.issue-list) .ops-header h2,.ops:has(.issue-list) .section-title h3,.ops:has(.issue-list) .section-title p{color:#e6edf7}.ops:has(.issue-list) .ops-header p,.ops:has(.issue-list) .issue-count{color:#91a4be}.ops:has(.issue-list) .progress-overview,.ops:has(.issue-list) .quick-add{background:#1b293e;border-color:#30425f;color:#dbe5f2}.ops:has(.issue-list) .overall-ring:after{background:#1b293e}.ops:has(.issue-list) .overall-ring{background:#293b56;color:#45d483}.ops:has(.issue-list) .overview-counts,.ops:has(.issue-list) .category-progress span,.ops:has(.issue-list) .category-progress small{color:#9db0c9}.ops:has(.issue-list) .quick-add b{color:#dbe5f2}.ops:has(.issue-list) .quick-add input,.ops:has(.issue-list) .quick-add select{background:#111b2d;border-color:#3a4d6a;color:#dbe5f2}.ops:has(.issue-list) .issue-dashboard article{background:#1b293e;border-color:#30425f}.ops:has(.issue-list) .issue-dashboard small,.ops:has(.issue-list) .issue-dashboard span{color:#9db0c9}.ops:has(.issue-list) .issue-dashboard strong{color:#f2f6fb}.ops:has(.issue-list) .issue-toolbar{border-bottom:1px solid #2a3a55}.ops:has(.issue-list) .category-filters button{background:#1b293e;border-color:#3a4d6a;color:#9db0c9}.ops:has(.issue-list) .category-filters button.active{background:#29486b;color:#fff;border-color:#4d91d8}.ops:has(.issue-list) .issue-list{gap:0;border:1px solid #30425f;border-radius:8px;overflow:hidden;background:#17243a}.ops:has(.issue-list) .issue-list:before{content:'狀態　　　分類　　　　項目　　　　　　　　　　　　　　　　時程　　　　　預計日　　　動作';display:block;padding:11px 16px;border-bottom:1px solid #30425f;color:#9db0c9;font-size:10px;letter-spacing:.3px;white-space:pre}.ops:has(.issue-list) .issue-card{display:grid;grid-template-columns:1fr;border:0;border-bottom:1px solid #293b55;border-radius:0;padding:12px 16px;background:#17243a;color:#dbe5f2}.ops:has(.issue-list) .issue-card:hover{background:#1d2e47}.ops:has(.issue-list) .issue-card:last-of-type{border-bottom:0}.ops:has(.issue-list) .issue-heading{display:flex;align-items:center}.ops:has(.issue-list) .issue-heading strong{color:#f2f6fb;font-size:12px}.ops:has(.issue-list) .issue-card p{margin:5px 0;color:#9db0c9;font-size:10px}.ops:has(.issue-list) .issue-card>.progress-track{height:4px;margin:6px 0;background:#2c3e59}.ops:has(.issue-list) .issue-dates{grid-template-columns:repeat(4,1fr);gap:6px}.ops:has(.issue-list) .issue-dates span{background:#1e3049;color:#9db0c9}.ops:has(.issue-list) .issue-dates strong{color:#e1eaf5}.ops:has(.issue-list) .category-badge{background:#30445f;color:#a9c7ef}.ops:has(.issue-list) .severity{background:#254a42;color:#63dfa0}@media(max-width:800px){.ops:has(.issue-list) .issue-list:before{font-size:9px}.ops:has(.issue-list) .issue-dates{grid-template-columns:1fr 1fr}}
.issue-actions{display:flex;gap:5px;margin-left:auto}.issue-actions button{border:1px solid #3a4d6a;border-radius:4px;background:transparent;color:#a9c7ef;padding:3px 7px;font-size:9px}.issue-actions button.danger{color:#f2a5a0;border-color:#71444d}.issue-actions button:hover{background:#29486b}
.ops:has(.issue-list){background:#fff;border-color:#dfe7e4;color:#263d3a}.ops:has(.issue-list) .ops-header,.ops:has(.issue-list) .ops-nav{border-color:#e4ece9}.ops:has(.issue-list) .ops-header h2,.ops:has(.issue-list) .section-title h3,.ops:has(.issue-list) .section-title p{color:#263d3a}.ops:has(.issue-list) .ops-header p,.ops:has(.issue-list) .issue-count{color:#758581}.ops:has(.issue-list) .progress-overview,.ops:has(.issue-list) .quick-add{background:#f8fbfa;border-color:#dfe7e4;color:#263d3a}.ops:has(.issue-list) .overall-ring:after{background:#f8fbfa}.ops:has(.issue-list) .overall-ring{background:#e7f2ef;color:#216b63}.ops:has(.issue-list) .overview-counts,.ops:has(.issue-list) .category-progress span,.ops:has(.issue-list) .category-progress small{color:#60736f}.ops:has(.issue-list) .quick-add b{color:#355c56}.ops:has(.issue-list) .quick-add input,.ops:has(.issue-list) .quick-add select{background:#fff;border-color:#d3e1dd;color:#355c56}.ops:has(.issue-list) .issue-dashboard article{background:#fff;border-color:#dfe7e4}.ops:has(.issue-list) .issue-dashboard small,.ops:has(.issue-list) .issue-dashboard span{color:#758581}.ops:has(.issue-list) .issue-dashboard strong{color:#194f49}.ops:has(.issue-list) .issue-toolbar{border-color:#e4ece9}.ops:has(.issue-list) .category-filters button{background:#fff;border-color:#d3e1dd;color:#60736f}.ops:has(.issue-list) .category-filters button.active{background:#e7f2ef;color:#1f655e;border-color:#8fbdb6}.ops:has(.issue-list) .issue-list{border-color:#dfe7e4;background:#fff}.ops:has(.issue-list) .issue-list:before{border-color:#e4ece9;color:#758581;background:#f8fbfa}.ops:has(.issue-list) .issue-card{border-color:#edf1f0;background:#fff;color:#263d3a}.ops:has(.issue-list) .issue-card:hover{background:#f8fbfa}.ops:has(.issue-list) .issue-heading strong{color:#263d3a}.ops:has(.issue-list) .issue-card p{color:#60736f}.ops:has(.issue-list) .issue-card>.progress-track{background:#e5eeeb}.ops:has(.issue-list) .issue-dates span{background:#f4f8f6;color:#60736f}.ops:has(.issue-list) .issue-dates strong{color:#294f4a}.ops:has(.issue-list) .category-badge{background:#edf2ff;color:#3d5e9b}.ops:has(.issue-list) .severity{background:#e7f2ef;color:#256a61}.issue-actions button{border-color:#c9d9d5;color:#286b63}.issue-actions button.danger{color:#a33d2c;border-color:#e4c2bc}.issue-actions button:hover{background:#e7f2ef}
.ops:has(.issue-list) .section-title h3{font-size:17px}.ops:has(.issue-list) .section-title p{font-size:13px;line-height:1.6}.ops:has(.issue-list) .quick-add b,.ops:has(.issue-list) .quick-add input,.ops:has(.issue-list) .quick-add select,.ops:has(.issue-list) .quick-add button{font-size:13px}.ops:has(.issue-list) .issue-list:before{font-size:12px;font-weight:700}.ops:has(.issue-list) .issue-heading strong{font-size:15px}.ops:has(.issue-list) .issue-heading>b{font-size:15px}.ops:has(.issue-list) .issue-card p{font-size:13px;line-height:1.6}.ops:has(.issue-list) .issue-dates{gap:8px}.ops:has(.issue-list) .issue-dates span{font-size:12px;padding:9px 10px}.ops:has(.issue-list) .issue-dates strong{font-size:13px;margin-top:4px}.ops:has(.issue-list) .category-badge,.ops:has(.issue-list) .severity{font-size:12px;padding:4px 7px}.issue-actions button{font-size:12px;padding:5px 9px}.ops:has(.issue-list) .category-filters button,.ops:has(.issue-list) .issue-count{font-size:12px}
</style>
