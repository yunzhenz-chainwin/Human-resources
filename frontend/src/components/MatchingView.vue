      ...
      <article v-for="item in people" :key="item.candidate.id" class="panel match-card" :class="{ pending: !item.match, expanded: expandedCandidateId === item.candidate.id }">
        <header class="match-row-summary">
          ...
          >
            ...
            <div v-if="item.match" class="score" :class="{ failed: !item.match.gate_passed }"><strong>{{ item.match.total_score.toFixed(1) }}<b>%</b></strong><small>初步規則適配 · {{ resultLabel(item) }}</small><em :data-confidence="item.match.confidence">資料 {{ Math.round(item.match.data_completeness * 100) }}% · 可信度{{ confidenceLabels[item.match.confidence] }}</em></div>
