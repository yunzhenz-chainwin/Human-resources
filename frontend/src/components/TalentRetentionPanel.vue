...
<template>
  <details class="retention-panel panel" data-testid="talent-retention-panel">
    <summary>
      <div><p>DATA RETENTION</p><h2>保存期限設定</h2><span>先設定公司預設，再從人才詳情指定 1～20 年；個別設定會優先於公司預設。</span></div>
      <span class="retention-state"><i></i>公司預設 {{ retentionYears }} 年</span>
      <b class="retention-toggle"><span>展開設定</span><span>收合設定</span></b>
    </summary>
    ...
    <div class="retention-body">
      ...
      <div class="individual-retention-guide">
        ...
        <div><strong>② 個別人才可另外指定</strong><p>在下方人才列點開詳情後選擇年限，畫面會同時顯示「公司預設／個別設定」與實際到期日。</p></div>
        <button class="button primary" type="button" @click="scrollToCandidates">前往人才清單 ↓</button>
      </div>
...
@media(max-width:760px){.retention-panel>summary{align-items:flex-start;flex-wrap:wrap}.retention-state{margin-left:0}.retention-toggle{margin-left:auto}.retention-policy,.retention-cleanup,.individual-retention-guide{align-items:flex-start;flex-direction:column}.retention-policy label,.retention-policy .button,.retention-cleanup .button,.individual-retention-guide .button{width:100%}.retention-policy input{flex:1}.purge-preview{grid-template-columns:1fr 1fr}.purge-preview .button{grid-column:1/-1}}@media(max-width:460px){.retention-panel>summary>div{flex-basis:100%}.retention-state{order:2}.retention-toggle{order:3}.purge-preview{grid-template-columns:1fr}}
</style>
